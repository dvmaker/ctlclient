...
const antifake = JSON.parse(fs.readFileSync('./ctl client/antis/antifake.json'))
  ...
  console.log('Error : %s', color(e, 'red'))
	...
	ctlclient.on('chat-update', async (mek) => {
		try {
			...
			const hora = moment.tz('Brasil/Sao_Paulo').format('HH:mm:ss')
...
case 'clonar':
					...
					var { jid, id, notify } = groupMembers.find(x => x.jid === mentioned)
					try {
